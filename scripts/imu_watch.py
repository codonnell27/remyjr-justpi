...
def start():
	...
	while not rospy.is_shutdown():
		...
		data = imu_data()
		data.north = int(math.degrees(math.atan2(mag_x, mag_y))) #this is a super rough estimate, doesn't account for tilt
		data.publish_count = publish_count
                data.is_level = findIfLevel()
		data.is_turning = findIfTurning()
                data.heading = heading
                data.pitch = pitch
                data.roll = roll
		data.heading_delta = heading_delta
		rospy.loginfo(data)
                pub.publish(data)
                publish_count += 1
